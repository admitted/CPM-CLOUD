...
<%
  ...
  ArrayList  Acc_Data_Month    = (ArrayList)session.getAttribute("Acc_Data_Month_" + Sid);
  int        sn         = 0; 
  String     Manage_List = "";
...
<form name="Acc_Data_Month"  action="Acc_Data.do" method="post" target="mFrame">
<div id="down_bg_2">
				...
				<tr height='30' valign='middle'>
							 <td width='100%' align='center' colspan=2>
										<table width="100%" border=1 cellPadding=0 cellSpacing=0 bordercolor="#3491D6" borderColorDark="#ffffff">
												...
												<%
												 if(Acc_Data_Month != null)
												 {
														Iterator iterator = Acc_Data_Month.iterator();
														while(iterator.hasNext())
														{
							     		 	...
							     		 	<tr>
													 <td colspan="7" class="table_deep_blue" >
											 			  <table width="100%" height="20"  border="0" cellpadding="0" cellspacing="0" >
										    			    <tr valign="bottom">
										      			       <td nowrap><%=currStatus.GetPageHtml("Acc_Data_Month")%></td>
										    			    </tr>			    		
														  </table>
													 </td>
												</tr>
...
function doSelect()
{
		Acc_Data_Month.BTime.value = Acc_Data_Month.BDate.value + " 00:00:00";
		Acc_Data_Month.submit();
}
...
function GoPage(pPage)
{
	...
	Acc_Data_Month.BTime.value = Acc_Data_Month.BDate.value + " 00:00:00";
	Acc_Data_Month.CurrPage.value = pPage;
	Acc_Data_Month.submit();
}
...
{	
	...
	if(confirm("确定导出?"))
  {
		if(window.XMLHttpRequest)
	  {
		...
		var url = "Acc_Data_Month_Export.do?Sid=<%=Sid%>&Id="+window.parent.frames.lFrame.document.getElementById('id').value+"&Level="+window.parent.frames.lFrame.document.getElementById('level').value+"&BTime="+Acc_Data_Month.BDate.value+" 00:00:00"+"&CurrPage=<%=currStatus.getCurrPage()%>";
